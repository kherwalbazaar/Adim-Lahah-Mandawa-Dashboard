...
export function buildCashbook(raw: RawMap, now: Date): CashbookData {
  const entries: CashbookEntry[] = toEntries(raw).map((e) => ({
    ...
    mode: String(e.mode ?? e.via ?? ""),
    remark: String(e.remark ?? e.description ?? ""),
    date: String(e.date ?? ""),
    type: e.type === "out" || e.type === "Expense" ? "out" : "in",
    createdAt: Number(e.createdAt ?? 0),
  }))
  const compareByDate = (a: CashbookEntry, b: CashbookEntry) =>
  ...
  let balance = 0
  const asc = [...entries].sort((a, b) => compareByDate(b, a))
  ...
  return {
    entries: entries.sort(compareByDate),
    metrics: { opening, monthIn, monthOut, totalOut, closing, monthKey },
    running,
    trend,
  }
}
